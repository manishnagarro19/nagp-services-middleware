# -*- coding: utf-8 -*-
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: order.proto
"""Generated protocol buffer code."""
from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database

# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()


DESCRIPTOR = _descriptor.FileDescriptor(
    name="order.proto",
    package="order",
    syntax="proto3",
    serialized_options=None,
    create_key=_descriptor._internal_create_key,
    serialized_pb=b'\n\x0border.proto\x12\x05order"F\n\x0cOrderRequest\x12\x12\n\nproduct_id\x18\x01 \x01(\x05\x12\x10\n\x08order_id\x18\x02 \x01(\x05\x12\x10\n\x08quantity\x18\x03 \x01(\x05"!\n\rOrderResponse\x12\x10\n\x08order_id\x18\x01 \x01(\x05\x32\x81\x01\n\x0cOrderService\x12\x37\n\nPlaceOrder\x12\x13.order.OrderRequest\x1a\x14.order.OrderResponse\x12\x38\n\x0bUpdateOrder\x12\x13.order.OrderRequest\x1a\x14.order.OrderResponseb\x06proto3',
)


_ORDERREQUEST = _descriptor.Descriptor(
    name="OrderRequest",
    full_name="order.OrderRequest",
    filename=None,
    file=DESCRIPTOR,
    containing_type=None,
    create_key=_descriptor._internal_create_key,
    fields=[
        _descriptor.FieldDescriptor(
            name="product_id",
            full_name="order.OrderRequest.product_id",
            index=0,
            number=1,
            type=5,
            cpp_type=1,
            label=1,
            has_default_value=False,
            default_value=0,
            message_type=None,
            enum_type=None,
            containing_type=None,
            is_extension=False,
            extension_scope=None,
            serialized_options=None,
            file=DESCRIPTOR,
            create_key=_descriptor._internal_create_key,
        ),
        _descriptor.FieldDescriptor(
            name="order_id",
            full_name="order.OrderRequest.order_id",
            index=1,
            number=2,
            type=5,
            cpp_type=1,
            label=1,
            has_default_value=False,
            default_value=0,
            message_type=None,
            enum_type=None,
            containing_type=None,
            is_extension=False,
            extension_scope=None,
            serialized_options=None,
            file=DESCRIPTOR,
            create_key=_descriptor._internal_create_key,
        ),
        _descriptor.FieldDescriptor(
            name="quantity",
            full_name="order.OrderRequest.quantity",
            index=2,
            number=3,
            type=5,
            cpp_type=1,
            label=1,
            has_default_value=False,
            default_value=0,
            message_type=None,
            enum_type=None,
            containing_type=None,
            is_extension=False,
            extension_scope=None,
            serialized_options=None,
            file=DESCRIPTOR,
            create_key=_descriptor._internal_create_key,
        ),
    ],
    extensions=[],
    nested_types=[],
    enum_types=[],
    serialized_options=None,
    is_extendable=False,
    syntax="proto3",
    extension_ranges=[],
    oneofs=[],
    serialized_start=22,
    serialized_end=92,
)


_ORDERRESPONSE = _descriptor.Descriptor(
    name="OrderResponse",
    full_name="order.OrderResponse",
    filename=None,
    file=DESCRIPTOR,
    containing_type=None,
    create_key=_descriptor._internal_create_key,
    fields=[
        _descriptor.FieldDescriptor(
            name="order_id",
            full_name="order.OrderResponse.order_id",
            index=0,
            number=1,
            type=5,
            cpp_type=1,
            label=1,
            has_default_value=False,
            default_value=0,
            message_type=None,
            enum_type=None,
            containing_type=None,
            is_extension=False,
            extension_scope=None,
            serialized_options=None,
            file=DESCRIPTOR,
            create_key=_descriptor._internal_create_key,
        ),
    ],
    extensions=[],
    nested_types=[],
    enum_types=[],
    serialized_options=None,
    is_extendable=False,
    syntax="proto3",
    extension_ranges=[],
    oneofs=[],
    serialized_start=94,
    serialized_end=127,
)

DESCRIPTOR.message_types_by_name["OrderRequest"] = _ORDERREQUEST
DESCRIPTOR.message_types_by_name["OrderResponse"] = _ORDERRESPONSE
_sym_db.RegisterFileDescriptor(DESCRIPTOR)

OrderRequest = _reflection.GeneratedProtocolMessageType(
    "OrderRequest",
    (_message.Message,),
    {
        "DESCRIPTOR": _ORDERREQUEST,
        "__module__": "order_pb2"
        # @@protoc_insertion_point(class_scope:order.OrderRequest)
    },
)
_sym_db.RegisterMessage(OrderRequest)

OrderResponse = _reflection.GeneratedProtocolMessageType(
    "OrderResponse",
    (_message.Message,),
    {
        "DESCRIPTOR": _ORDERRESPONSE,
        "__module__": "order_pb2"
        # @@protoc_insertion_point(class_scope:order.OrderResponse)
    },
)
_sym_db.RegisterMessage(OrderResponse)


_ORDERSERVICE = _descriptor.ServiceDescriptor(
    name="OrderService",
    full_name="order.OrderService",
    file=DESCRIPTOR,
    index=0,
    serialized_options=None,
    create_key=_descriptor._internal_create_key,
    serialized_start=130,
    serialized_end=259,
    methods=[
        _descriptor.MethodDescriptor(
            name="PlaceOrder",
            full_name="order.OrderService.PlaceOrder",
            index=0,
            containing_service=None,
            input_type=_ORDERREQUEST,
            output_type=_ORDERRESPONSE,
            serialized_options=None,
            create_key=_descriptor._internal_create_key,
        ),
        _descriptor.MethodDescriptor(
            name="UpdateOrder",
            full_name="order.OrderService.UpdateOrder",
            index=1,
            containing_service=None,
            input_type=_ORDERREQUEST,
            output_type=_ORDERRESPONSE,
            serialized_options=None,
            create_key=_descriptor._internal_create_key,
        ),
    ],
)
_sym_db.RegisterServiceDescriptor(_ORDERSERVICE)

DESCRIPTOR.services_by_name["OrderService"] = _ORDERSERVICE

# @@protoc_insertion_point(module_scope)
